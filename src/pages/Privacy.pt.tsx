  ...
  return (
    <div className="container privacy">
      ...
      <p>
        ...
        recolhe dados pessoais «sensíveis» ou biológicos como fotos, localização
        GPS, contactos ou outros dados desnecessários ao uso da app, com
        exceção dos dados de saúde e atividade física que o utilizador pode
        opcionalmente partilhar através da integração com Apple Health
        (HealthKit) ou Health Connect descrita na Secção 4.
      </p>
      <p>
      ...
      <h2>4. Dados de saúde e atividade física</h2>
      <p>
        A GymBroTools oferece uma integração opcional com Apple Health
        (HealthKit) no iOS e Health Connect no Android, permitindo aos
        utilizadores sincronizar dados de saúde e atividade física entre a
        GymBroTools e a plataforma de saúde do seu dispositivo. Esta
        integração é totalmente facultativa e requer o consentimento explícito
        do utilizador, concedido através do diálogo nativo de autorização do
        sistema operativo.
      </p>
      <p>
        <strong>
          Dados que podemos ler da plataforma de saúde do dispositivo:
        </strong>
        {" "}peso corporal, altura, percentagem de gordura corporal e massa
        magra. Estes dados podem ter origem noutros dispositivos ou apps (por
        exemplo, balanças inteligentes) ligados à plataforma de saúde.
      </p>
      <p>
        <strong>
          Dados que podemos escrever na plataforma de saúde do dispositivo:
        </strong>
        {" "}peso corporal, altura, percentagem de gordura corporal, massa
        magra, IMC (Índice de Massa Corporal), TMB (Taxa Metabólica Basal) e
        perímetro da cintura; sessões de treino incluindo tipo de exercício,
        duração, calorias queimadas, distância e frequência cardíaca quando
        disponível.
      </p>
      <p>
        <strong>Finalidades do tratamento dos dados de saúde:</strong> os dados
        de saúde são utilizados exclusivamente para (a) importar medidas
        corporais de dispositivos externos (por exemplo, balanças inteligentes)
        para as consolidar no perfil fitness do utilizador dentro da
        GymBroTools; (b) exportar treinos e medidas corporais registados na app
        para a plataforma de saúde do dispositivo, para que fiquem disponíveis
        ao utilizador num local centralizado; e (c) calcular métricas derivadas
        como o IMC, o FFMI (Índice de Massa Isenta de Gordura) e a relação
        cintura‑altura.
      </p>
      <p>
        <strong>Base jurídica (art. 9.º RGPD):</strong> os dados de saúde
        constituem uma «categoria especial» de dados pessoais nos termos do
        art. 9.º do RGPD. A base jurídica para o tratamento destes dados é o
        consentimento explícito do utilizador (art. 9.º(2)(a) RGPD), obtido
        através do diálogo nativo de autorização do sistema operativo (ecrã de
        permissões do Apple Health no iOS; ecrã de permissões do Health
        Connect no Android). As permissões são granulares: o utilizador pode
        autorizar ou negar o acesso a cada tipo de dado individualmente.
        Apenas são solicitados os tipos de dados estritamente necessários para
        fornecer as funcionalidades de integração fitness, em conformidade com
        o princípio da minimização dos dados.
      </p>
      <p>
        <strong>Conservação e partilha:</strong> os dados de saúde
        sincronizados através do Apple Health ou Health Connect são transmitidos
        para os servidores da GymBroTools como parte das medidas corporais do
        utilizador, protegidos por autenticação e cifragem em trânsito
        (HTTPS). Os dados de saúde <strong>não</strong> são vendidos a
        terceiros, partilhados com terceiros para fins publicitários ou de
        marketing, utilizados para perfilagem, nem empregues para quaisquer
        fins que não as funcionalidades da app acima descritas.
      </p>
      <p>
        <strong>Controlo do utilizador:</strong> a sincronização dos dados de
        saúde está desativada por predefinição. O utilizador pode ativá-la ou
        desativá-la a qualquer momento nas Definições da app. No iOS, as
        permissões do Apple Health podem ser geridas em Definições &gt; Saúde
        &gt; Acesso a dados e dispositivos. No Android, o utilizador pode
        revogar as permissões do Health Connect diretamente nas definições do
        dispositivo (Definições &gt; Health Connect &gt; Permissões de apps).
        Após a revogação, a GymBroTools deixará de ler e escrever dados de
        saúde. A eliminação da conta implica a remoção de todos os dados dos
        servidores da GymBroTools; os dados já escritos no Apple Health ou
        Health Connect permanecem sob o controlo do utilizador no seu próprio
        dispositivo.
      </p>
      <p>
        <strong>Conformidade com as plataformas:</strong> a GymBroTools cumpre
        as diretrizes da Apple para a utilização do HealthKit e as políticas
        da Google para programadores do Health Connect. Os dados de saúde são
        tratados em conformidade com o RGPD, a legislação aplicável em matéria
        de proteção de dados de saúde e os requisitos específicos das
        plataformas para apps que acedem a dados de saúde.
      </p>
      ...
      <h2>6. Base jurídica do tratamento</h2>
      <p>
      ...
      <h2>7. Modalidades do tratamento e prazos de conservação</h2>
      <p>
      ...
      <h2>10. Responsável pelo tratamento e contactos</h2>
      <p>
      ...
      <h2>11. Alterações e atualizações da Política de Privacidade</h2>
      <p>
