export default function PrivacyFr() {
  ...
  return (
    <div className="container privacy">
      ...
      <p>
        <strong>IMPORTANT:</strong> l'app est destinée aux utilisateurs âgés de
        13 ans ou plus. L'utilisation de l'app par des enfants de moins de 13
      ...
      <h2>2. Mineurs</h2>
      <p>
        <strong>Enfants de moins de 13 ans:</strong> GymBroTools n'est pas
        destinée aux enfants de moins de 13 ans et nous ne collectons pas
        intentionnellement de données personnelles auprès d'enfants de cet âge.
        Si un parent ou tuteur estime qu'un enfant de moins de 13 ans a fourni
        des données personnelles via l'app, il est invité à nous contacter
        immédiatement à{" "}
        <a href="mailto:marcocrupi@hotmail.it">marcocrupi@hotmail.it</a> pour
        demander leur suppression.
      </p>
      <p>
        <strong>Utilisateurs de 13 à 17 ans:</strong> les mineurs (13-17 ans)
        peuvent utiliser l'app. Une supervision parentale ou d'un tuteur est
        recommandée pour garantir une utilisation responsable de l'application.
      </p>

      <h2>3. Types de données collectées</h2>
      <p>
      ...
      <p>
        ...
        des photos, la localisation GPS, les contacts ou d’autres données non
        nécessaires à l’utilisation de l’app, à l’exception des données de
        santé et d’activité physique que l’utilisateur peut facultativement
        partager via l’intégration avec Apple Health (HealthKit) ou Health
        Connect décrite à la Section 4.
      </p>
      <p>
      ...
      <h2>4. Données de santé et d'activité physique</h2>
      <p>
        GymBroTools propose une intégration optionnelle avec Apple Health
        (HealthKit) sur iOS et Health Connect sur Android, permettant aux
        utilisateurs de synchroniser des données de santé et d'activité
        physique entre GymBroTools et la plateforme de santé de leur appareil.
        Cette intégration est entièrement facultative et nécessite le
        consentement explicite de l'utilisateur, accordé via la boîte de
        dialogue native d'autorisation du système d'exploitation.
      </p>
      <p>
        <strong>
          Données que nous pouvons lire depuis la plateforme de santé de
          l'appareil:
        </strong>
        {" "}poids corporel, taille, pourcentage de masse grasse et masse
        maigre. Ces données peuvent provenir d'autres appareils ou applications
        (p. ex. balances connectées) liés à la plateforme de santé.
      </p>
      <p>
        <strong>
          Données que nous pouvons écrire sur la plateforme de santé de
          l'appareil:
        </strong>
        {" "}poids corporel, taille, pourcentage de masse grasse, masse maigre,
        IMC (Indice de Masse Corporelle), MB (Métabolisme de Base) et tour de
        taille; séances d'entraînement incluant le type d'exercice, la durée,
        les calories brûlées, la distance et la fréquence cardiaque lorsque
        disponible.
      </p>
      <p>
        <strong>Finalités du traitement des données de santé:</strong> les
        données de santé sont utilisées exclusivement pour (a) importer des
        mesures corporelles depuis des appareils externes (p. ex. balances
        connectées) afin de les consolider dans le profil fitness de
        l'utilisateur au sein de GymBroTools; (b) exporter les entraînements
        et mesures corporelles enregistrés dans l'app vers la plateforme de
        santé de l'appareil, afin qu'ils soient disponibles en un seul
        endroit centralisé; et (c) calculer des métriques dérivées telles que
        l'IMC, le FFMI (Indice de Masse Maigre) et le rapport taille‑hauteur.
      </p>
      <p>
        <strong>Base juridique (art. 9 RGPD):</strong> les données de santé
        constituent une « catégorie particulière » de données personnelles au
        sens de l'art. 9 du RGPD. La base juridique du traitement de ces
        données est le consentement explicite de l'utilisateur (art. 9(2)(a)
        RGPD), obtenu via la boîte de dialogue native d'autorisation du
        système d'exploitation (écran de permissions d'Apple Health sur iOS;
        écran de permissions de Health Connect sur Android). Les permissions
        sont granulaires: l'utilisateur peut autoriser ou refuser l'accès à
        chaque type de données individuellement. Seuls les types de données
        strictement nécessaires pour fournir les fonctionnalités d'intégration
        fitness sont demandés, conformément au principe de minimisation des
        données.
      </p>
      <p>
        <strong>Conservation et partage:</strong> les données de santé
        synchronisées via Apple Health ou Health Connect sont transmises aux
        serveurs de GymBroTools dans le cadre des mesures corporelles de
        l'utilisateur, protégées par l'authentification et le chiffrement en
        transit (HTTPS). Les données de santé ne sont <strong>pas</strong>
        {" "}vendues à des tiers, partagées avec des tiers à des fins
        publicitaires ou marketing, utilisées à des fins de profilage, ni
        employées à toute autre fin que les fonctionnalités de l'app décrites
        ci‑dessus.
      </p>
      <p>
        <strong>Contrôle de l'utilisateur:</strong> la synchronisation des
        données de santé est désactivée par défaut. L'utilisateur peut
        l'activer ou la désactiver à tout moment depuis les Réglages de l'app.
        Sur iOS, les permissions d'Apple Health peuvent être gérées depuis
        Réglages &gt; Santé &gt; Accès aux données et appareils. Sur Android,
        l'utilisateur peut révoquer les permissions de Health Connect
        directement depuis les paramètres de l'appareil (Paramètres &gt;
        Health Connect &gt; Autorisations des apps). À la suite de la
        révocation, GymBroTools ne lira ni n'écrira plus de données de santé.
        La suppression du compte entraîne l'effacement de toutes les données
        des serveurs de GymBroTools; les données déjà écrites sur Apple Health
        ou Health Connect restent sous le contrôle de l'utilisateur sur son
        propre appareil.
      </p>
      <p>
        <strong>Conformité aux plateformes:</strong> GymBroTools respecte les
        directives d'Apple pour l'utilisation de HealthKit et les politiques de
        Google pour les développeurs de Health Connect. Les données de santé
        sont traitées conformément au RGPD, aux lois applicables en matière de
        protection des données de santé et aux exigences spécifiques des
        plateformes pour les apps accédant à des données de santé.
      </p>
      ...
      <h2>6. Base juridique du traitement</h2>
      <p>
      ...
      <h2>7. Modalités de traitement et durée de conservation</h2>
      <p>
      ...
      <h2>9. Sécurité et protection des données</h2>
      <p>
      ...
      <h2>10. Responsable du traitement et contact</h2>
      <p>
      ...
      <h2>
        11. Modifications et mises à jour de la présente Politique de
        confidentialité
      </h2>
      <p>
